...
const char * const PICACOIN_CONF_FILENAME = "picacoin.conf";
const char * const PICACOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/picacoin/picacoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //picacoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", PICACOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Picacoin
    // macOS: ~/Library/Application Support/Picacoin
    // Unix-like: ~/.picacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Picacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Picacoin";
#else
    // Unix-like
    return pathRet / ".picacoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PICACOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Picacoin Core copyright is not removed by accident
    if (copyright_devs.find("Picacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Picacoin Core developers";
    }
