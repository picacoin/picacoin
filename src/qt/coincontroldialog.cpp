...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of picaros so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    // how many picaros the estimated fee can vary per byte we guess wrong
