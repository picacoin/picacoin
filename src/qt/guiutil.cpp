...
#include <qt/picacoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Picacoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new PicacoinAddressEntryValidator(parent));
    widget->setCheckValidator(new PicacoinAddressCheckValidator(parent));
}

bool parsePicacoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no picacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("picacoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PicacoinUnits::parse(PicacoinUnits::PIC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePicacoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsePicacoinURI(uriInstance, out);
}

QString formatPicacoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("picacoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PicacoinUnits::format(PicacoinUnits::PIC, info.amount, false, PicacoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPicacoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", PICACOIN_CONF_FILENAME));
    ...
    /* Open picacoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Picacoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Picacoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Picacoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Picacoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "picacoin.desktop";
    return GetAutostartDir() / strprintf("picacoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a picacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Picacoin\n";
        else
            optionFile << strprintf("Name=Picacoin (%s)\n", chain);
