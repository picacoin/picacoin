This directory contains the source code for the Picacoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Picacoin UI.
...
- ClientModel is used by the main application `picacoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a picacoin:... link from a browser).
...
* UI elements like PicacoinAmountField, which inherit from QWidget.
* `picacoinstrings.cpp`: automatically generated
* `picacoinunits.(h/cpp)`: PIC / mPIC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/picacoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `picacoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `picacoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `picacoin-qt`
9. You're all set! Start developing, building, and debugging the Picacoin Core GUI
