...
from test_framework.util import assert_equal, assert_raises_rpc_error, picaro_round
...
def make_utxo(node, amount, confirmed=True, scriptPubKey=DUMMY_P2WPKH_SCRIPT):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < picaro_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, picaro_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(PicacoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 PIC - so this attempt is rejected.
        dbl_tx = CTransaction()
