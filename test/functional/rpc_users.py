...
class HTTPBasicsTest(PicacoinTestFramework):
    ...
    def setup_chain(self):
        super().setup_chain()
        #Append rpcauth to picacoin.conf before initialization
        ...
        with open(os.path.join(get_datadir_path(self.options.tmpdir, 0), "picacoin.conf"), 'a', encoding='utf8') as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(os.path.join(get_datadir_path(self.options.tmpdir, 1), "picacoin.conf"), 'a', encoding='utf8') as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
