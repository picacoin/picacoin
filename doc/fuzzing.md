# Fuzzing Picacoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Picacoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/picacoin/picacoin
$ cd picacoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/picacoin/picacoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/picacoin/picacoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/picacoin/picacoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/picacoin/picacoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`picacoin-core/qa-assets`](https://github.com/picacoin-core/qa-assets) repo.

To fuzz `process_message` using the [`picacoin-core/qa-assets`](https://github.com/picacoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/picacoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`picacoin-core/qa-assets`](https://github.com/picacoin-core/qa-assets) repo.

Every single pull request submitted against the Picacoin Core repo is automatically tested against all inputs in the [`picacoin-core/qa-assets`](https://github.com/picacoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Picacoin Core more robust.
...
with certain assembly code from Picacoin Core's code. See [developer notes on sanitizers](https://github.com/picacoin/picacoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Picacoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Picacoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Picacoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Picacoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Picacoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Picacoin
Core without having to write any custom fuzzing harness. The `picacoind` server
...
```sh
$ mkdir picacoin-honggfuzz-p2p/
$ cd picacoin-honggfuzz-p2p/
$ git clone https://github.com/picacoin/picacoin
$ cd picacoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/picacoind.cpp b/src/picacoind.cpp
...
Picacoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/picacoin-core)
...
because Picacoin is a live P2P network, and not just standalone local software,
