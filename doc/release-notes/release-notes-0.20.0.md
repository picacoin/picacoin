...
Picacoin Core version 0.20.0 is now available from:

  <https://picacoincore.org/bin/picacoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Picacoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Picacoin Core on
unsupported systems.

From Picacoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Picacoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `PICACOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Picacoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Picacoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Picacoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Picacoin Core. Picacoin Core logs debug messages
...
- OpenSSL is no longer used by Picacoin Core.  (#17265)

- BIP70 support has been fully removed from Picacoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/picacoin/picacoin/issues/16599),
  [PR #16702](https://github.com/picacoin/picacoin/pull/16702), and the
  `picacoind help` for more information.  This option is experimental and
...
- All custom settings configured when Picacoin Core starts are now
...
- The "Start Picacoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://picacoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Picacoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Picacoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/picacoin-core/HWI) for
  signing.  Future versions of Picacoin Core should support a GUI option
...
- Picacoin Core's automatically-generated source code documentation is
  now available at https://doxygen.picacoincore.org.  (#17596)
...
- The `picacoin-cli` utility used with the `-getinfo` parameter now
...
- The `picacoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `picacoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_picacoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBPICACOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_picacoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBPICACOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for picacoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate picacoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest picacoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_picacoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (picacoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone picacoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Picacoin Core files (hebasto)
...
- #17119 Fix broken picacoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from picacoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.picacoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to picacoin-wallet (hel-o)
...
- #18341 Replace remaining literal PIC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On picacoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from picacoin-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/picacoin/picacoin/).
