...
Picacoin Core version 0.21.0 is now available from:

  <https://picacoincore.org/bin/picacoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/picacoin/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Picacoin Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Picacoin Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/picacoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/picacoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/picacoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/picacoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/picacoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `picacoin-cli -netinfo` command provides a network peer connections
...
- A new `picacoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `picacoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `picacoin-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Picacoin Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `picacoin-wallet` tool which performs the salvage
...
Picacoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`picacoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Picacoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `picacoin-cli createwallet` or `picacoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/picacoin/picacoin/issues).
...
Users who already have a Picacoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Picacoin Core.
...
The change to using descriptors changes the default derivation paths used by Picacoin Core
...
- A new `fee_rate` parameter/option denominated in picaros per vbyte (sat/vB)
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Picacoin Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `picacoin.conf`
...
- #19284 Add seed.picacoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in picacoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add picacoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in picacoin-cli (laanwj)
...
- #18297 Use pkg-config in PICACOIN_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for picacoin-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Picacoin Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrepicacoin)
...
- #18817 Document differences in picacoind and picacoin-qt locale handling (practicalswift)
