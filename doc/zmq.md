...
The Picacoin Core daemon can be configured to act as a trusted "border
router", implementing the picacoin wire protocol and relay, making
...
The ZeroMQ feature in Picacoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building picacoind:
...
For instance:
    ...
    $ picacoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/picacoind.tx.raw \
...
These options can also be provided in picacoin.conf.
...
From the perspective of picacoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into picacoind directly. Furthermore, no information is
...
using. Picacoind appends an up-counting sequence number to each
