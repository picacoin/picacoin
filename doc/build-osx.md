...
This guide describes how to build picacoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Picacoin Core from source.
...
### 4. Clone Picacoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Picacoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `picacoind` or  `picacoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/picacoin/picacoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Picacoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/picacoin/picacoin/issues/7714).
...
You can deploy a `.dmg` containing the Picacoin Core application using `make deploy`.
...
## Building Picacoin Core

### 1. Configuration

There are many ways to configure Picacoin Core, here are a few common examples:
...
Run the following in your terminal to compile Picacoin Core:
...
## Running Picacoin Core

Picacoin Core should now be available at `./src/picacoind`.
If you compiled support for the GUI, it should be available at `./src/qt/picacoin-qt`.

The first time you run `picacoind` or `picacoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Picacoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Picacoin"

touch "/Users/${USER}/Library/Application Support/Picacoin/picacoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Picacoin/picacoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Picacoin/debug.log
```

## Other commands:

```shell
./src/picacoind -daemon      # Starts the picacoin daemon.
./src/picacoin-cli --help    # Outputs a list of command-line options.
./src/picacoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/picacoin-qt -server # Starts the picacoin-qt server mode, allows picacoin-cli control
```
